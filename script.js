...
let usedIndices = []; // Hier wird das Array für die verwendeten Indizes hinzugefügt
...
function loadNextEvent() {
    if (roundsPlayed < maxRounds) {
        // Wähle einen zufälligen Index, der noch nicht verwendet wurde
        let randomIndex;
        do {
            randomIndex = Math.floor(Math.random() * events.length);
        } while (usedIndices.includes(randomIndex));

        // Speichere den Index, damit er nicht wiederholt wird
        usedIndices.push(randomIndex);

        // Aktualisiere die Anzeige mit dem zufällig ausgewählten Ereignis
        document.getElementById("event-text").innerText = events[randomIndex].event;
        document.getElementById("year-input").value = '';
        document.getElementById("result-text").innerText = '';
    } else {
        // Zeige den Gesamtscore nach allen Runden an
